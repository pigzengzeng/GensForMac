...
- 屏幕交互式按键设置：
  - 键盘 + 手柄双列映射，任意功能键可重绑定
  - 每个端口独立选择输入源：**键盘 / 指定某个手柄 / 无（NONE）**；同一个手柄可绑定到多个端口实现「一控二」（清版、飞行射击等多人共用一台设备的玩法）
  - 8 个逻辑端口；MD 模拟统一按 6 键手柄处理（向下兼容 3 键游戏），不再单独提供 3 键/6 键切换
...
- **MULTITAP 模式**（菜单栏 Controllers → Configure Controls… → MULTITAP 行）：
  - **Off（默认）**：标准 2 人布局，1P / 2P 各占一个主机端口，普通双人/对战游戏照常
  - **Team Player（端口 1 / 端口 2）**：把 PORT1–4 映射到同一主机端口的 4 个子槽并强制启用 Sega Team Player。幽游白书、街头霸王等「按 START 自动加入、无人数选择」的对战游戏即可 1P–4P 同场——把 PORT1–4 各绑一个手柄，各自按手柄 START 加入即可（绝大多数 MD 四人游戏用端口 2 的 Team Player，若 3P/4P 不进再试端口 1）
- 设置自动持久化到 `~/.gensmacrc`
